...
export const SubscriptionForm = ({ onSubmit, onClose, initialSubscription }: Props) => {
  const [loading, setLoading] = useState(false);

  const initialValues: Subscription = initialSubscription || {
    id: 0,
    value: 0,
    startDate: dayjs().startOf('day').toDate(),
  };

  const handleSubmitFormik = async (values: Subscription) => {
    setLoading(true);
    try {
